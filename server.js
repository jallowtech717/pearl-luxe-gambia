...
app.get("/api/products",async(_,res)=>{try{if(!db)return res.status(503).json({error:"Store database unavailable."});const out=await db.execute("SELECT * FROM products WHERE archived=0 ORDER BY featured DESC,created_at DESC"),items=[];for(const r of out.rows){const imgs=(await db.execute({sql:"SELECT image_data FROM product_images WHERE product_id=? ORDER BY sort_order LIMIT 5",args:[r.id]})).rows.map(x=>String(x.image_data));items.push({...r,id:Number(r.id),price:Number(r.price),salePrice:r.sale_price==null?null:Number(r.sale_price),stock:Number(r.stock),featured:Boolean(r.featured),image:imgs[0]||null,images:imgs})}res.json(items)}catch(e){console.error(e);res.status(500).json({error:"Could not load products."})}});
...
app.get("/api/orders/track",async(req,res)=>{try{if(!db)return res.status(503).json({error:"Tracking is temporarily unavailable."});const order=clean(req.query.order,30).toUpperCase(),phone=normalizePhone(clean(req.query.phone,30)),out=await db.execute({sql:"SELECT order_number,status,created_at FROM orders WHERE order_number=? AND phone=?",args:[order,phone]});if(!out.rows.length)return res.status(404).json({error:"No matching order was found."});res.json({orderNumber:out.rows[0].order_number,status:out.rows[0].status,updatedAt:out.rows[0].created_at})}catch(e){res.status(500).json({error:"Tracking is temporarily unavailable."})}});
